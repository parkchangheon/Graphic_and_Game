...
bool AS2R_Second_Lever::SecondLeverResult(char answer) {
	// 입력을 받는다.
	bool input; // 델리게이트로answer 전달하고 결과값 받아서 비교

	if (Fuc_DeleSingle.IsBound() == true)  //델리게이트가 연결이 되었다면, 
		Fuc_DeleSingle.Execute(); //실행해준다.
	// 정확히는 델리게이트에 등록된 CallBack 함수를 호출하는 것.
