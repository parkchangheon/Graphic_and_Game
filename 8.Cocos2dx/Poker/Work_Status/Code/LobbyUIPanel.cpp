...
void LobbyUIPanel::InitPanel()
{
	...
	const float fWheelScale = 0.15f;
	getImage("wheel_back_outer")->setScale(fWheelScale);
	getImage("wheel_spin_10")->setScale(fWheelScale);
	getImage("wheel_center_pin")->setScale(fWheelScale);
...
void LobbyUIPanel::InitImage()
{
	...
	mGoMenu_3->setFontSize(20);	// 서바이벌토너먼트 문자 양이 많아 줄임
...
{	
	...
	if (GameDataManager::getSingletonPtr()->ShoulUpdateSurvivalInfo && GameDataManager::getSingletonPtr()->canUpdateSurvival()) {
		time_t timer = time(NULL);
		struct tm* t;
		t = localtime(&timer);
		GameDataManager::getSingletonPtr()->SurvivalUpdateReqTime = mktime(t);
		WebService::getSingletonPtr()->SurvivalInfoReq();
	}
