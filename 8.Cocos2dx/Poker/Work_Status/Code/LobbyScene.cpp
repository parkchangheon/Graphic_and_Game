...
void LobbyScene::onStart()
{
	...
	else
	{
		...
		if (params.isMember("LOBBY_UI_CHANNELLIST")) {
			//type = NotiDataManager::getSingleton().getEnterChannelNoti();		//chanenlist â �㶧 ����.
			mNotiType = NotiData_eNotiType_NoType;
...
void LobbyScene::onUpdate(float dt)
{
	if (CCmdQueue::getSingletonPtr())
		CCmdQueue::getSingleton().update(this);	//�ڹ� ��� ���޿�.
...
bool LobbyScene::handling(STCMD &stCmd)
{
	...
	if (auto* pstIt = castCMD<STCMD_IAP_ONESTORE_REQUEST_RESULT>(&stCmd))
	{
		...
		if (res.isSucess) {
			...
			//성공했을때
			sendW_RePayReq(res.tid, res.txid, res.receipt);

		}
		else {
			CCLog("STCMD_IAP_REQUEST_RESULT Nothing");
...
void LobbyScene::sendW_RePayReq(string tID, string txId, string receipt)
{
...
#endif
	HttpMessage msg(req, HttpResponseHandler(LobbyScene::recvW_RePayRes), this);
	HttpManager::getSingleton().send(msg);
}

void LobbyScene::recvW_RePayRes(HttpMessage* msg)
{
	...
	bool beforeFirstBuy = PokerRecordHelper::isFirstBonus(stoi(globalPid));
	...
	CCLog("---------------------------------------------------------");
	bool afterFirstBuy = PokerRecordHelper::isFirstBonus(stoi(globalPid));
	...
	if (res.success()) {
		...
		if (res.has_playerinfo())
		{
			...
			//상품 이름
			PanelHelper::pushRubyRewardPanel(getRewardRuby, stoi(globalPid), isFirstBuy);
			GameDataManager::getSingleton().setPlayerInfo(res.playerinfo());
...
void LobbyScene::getPPID(string pid)
{
	globalPid = pid;
}
